...
def get_product(product_id: int, days: int = 30, db: Session = Depends(get_db)):
...
async def create_product(product: ProductCreate, db: Session = Depends(get_db)):
    ...
    existing = crud.get_product_by_url(db, product.url)
    if existing:
        raise HTTPException(status_code=400, detail="Product already exists")
    
    # Create product
    db_product = crud.create_product(db, product.url, product.name, product.currency)
...
async def check_product_price(product_id: int, db: Session = Depends(get_db)):
