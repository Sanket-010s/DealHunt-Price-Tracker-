...
# Product CRUD
def create_product(db: Session, url: str, name: str, currency: str = "USD") -> Product:
    product = Product(url=url, name=name, currency=currency)
    db.add(product)
    db.commit()
    db.refresh(product)
    return product
...
def create_alert(db: Session, product_id: int, alert_type: AlertType, 
                 target_price: Optional[float] = None, percentage_drop: Optional[float] = None) -> Alert:
    alert = Alert(
